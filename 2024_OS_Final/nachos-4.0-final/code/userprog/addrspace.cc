...
bool AddrSpace::Load(char* fileName) {
    ...
    if (noffH.code.size > 0) {
        ...
        executable->ReadAt(&(kernel->machine->mainMemory[memoryAddr]), noffH.code.size, noffH.code.inFileAddr);
    }
    if (noffH.initData.size > 0) {
        ...
        executable->ReadAt(&(kernel->machine->mainMemory[memoryAddr]), noffH.initData.size, noffH.initData.inFileAddr);
    }
